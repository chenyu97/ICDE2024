...
for platform in range(N):
    # 进行 25 次优化
    row_profit = []
    for fixed_value in range(1, int(Fn_array[platform]) + 1):
        # 初始猜测值
        x = np.zeros((N, M))
        for i in range(N):
            for j in range(M):
        ...
        # 使用 minimize 函数进行优化
        result = minimize(objective_function, x0, constraints=my_constraints)
        ...
        row_profit.append(-1 * result.fun)
    rows_profit.append(row_profit)

with open('./result/Pareto_efficiency.csv', 'w', newline='') as csvfile:
