...
B_array_copy = [i for i in range(N)]
...
def callback_function(row_utility):
    def callback(xk):
        row_utility.append(objective_function(xk) * (-1))
    return callback


ratio = [10 / 15.0, 15 / 15.0, 20 / 15.0, 25 / 15.0]

rows_utility = []
for i in range(len(ratio)):
    row_utility = []
    for j in range(N):
        B_array[j] = B_array_copy[j] * ratio[i]
    ...
    x0 = x.flatten()
    row_utility.append(objective_function(x0) * (-1))
    ...
    result = minimize(objective_function, x0, constraints=my_constraints, callback = callback_function(row_utility))
...
with open('./result/MACRO_varing_B.csv', 'w', newline='') as csvfile:
