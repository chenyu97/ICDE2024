...
with open('./dataset/re_parameter_resolution_360p.csv', 'r') as csvfile:
    csvreader = csv.reader(csvfile)
    for row in csvreader:
        data.append([float(value) for value in row])
for j in range(M):
...
def callback_function(theta):
    def callback(xk):
        ...
        with open('./result_VariousResolution/strategy_ADMM.csv', 'a', newline='') as csvfile:
        ...
        with open('./result_VariousResolution/accuracy_ADMM.csv', 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
        ...
        with open('./result_VariousResolution/utility_ADMM.csv', 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
...
def write_random_initial(xk, theta):
    ...
    for i in range(N):
        for j in range(M):
            row_strategy.append(xk[i * M + j])
    rows_strategy.append(row_strategy)
    with open('./result_VariousResolution/strategy_ADMM.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerows(rows_strategy)
    # accuracy
    row_accuracy = []
    ...
    for i in range(N):
        for j in range(M):
            row_accuracy.append(accuracy_func(xk[i * M + j], a_fr_array[i, j], b_fr_array[i, j], c_fr_array[i, j]) * \
                                accuracy_func(re_array[j], a_re_array[i, j], b_re_array[i, j], c_re_array[i, j]))
    rows_accuracy.append(row_accuracy)
    with open('./result_VariousResolution/accuracy_ADMM.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerows(rows_accuracy)
    # utility
    row_utility = []
    ...
    for i in range(N):
        for j in range(M):
            row_utility.append(
                ...
                Energy(xk[i * M + j], i, j) - theta[i, j] * xk[i * M + j])
    rows_utility.append(row_utility)
    with open('./result_VariousResolution/utility_ADMM.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerows(rows_utility)
