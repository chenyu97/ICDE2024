...
C_array_copy = [i for i in range(M)]
...
ratio = [3 / 15.0, 9 / 15.0, 15 / 15.0, 21 / 15.0]

rows_utility = []
for i in range(len(ratio)):
    row_utility = []
    for j in range(M):
        C_array[j] = C_array_copy[j] * ratio[i]
...
with open('./result/MACRO_varing_C.csv', 'w', newline='') as csvfile:
